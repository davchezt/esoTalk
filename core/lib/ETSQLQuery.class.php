...
public function get()
{
	...
	//$query = preg_replace('/(:[A-Za-z0-9_]+)/e', 'array_key_exists("$1", $this->parameters) ? ET::$database->escapeValue($this->parameters["$1"][0], $this->parameters["$1"][1]) : "$1"', $query);
	$query = preg_replace_callback('/(:[A-Za-z0-9_]+)/', function ($m) {
		return array_key_exists($m[1], $this->parameters) ? ET::$database->escapeValue($this->parameters[$m[1]][0], $this->parameters[$m[1]][1]) : $m[1];
	}, $query);
	return $query;
}
