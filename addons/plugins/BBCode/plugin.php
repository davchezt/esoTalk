 ...
 */
        ...
        while (preg_match($regexp, $sender->content)) {
            if ($sender->inline) {
                $sender->content = preg_replace_callback('/(.*)^\s*\[code\]\n?(.*?)\n?\[\/code]$/ims', function ($m) {
                    $hideInline = function (&$inlineFixedContents, $contents) {
                ...
                $sender->content = preg_replace_callback('/(.*)^\s*\[code\]\n?(.*?)\n?\[\/code]$/ims', function($m) {
                    $hideBlock = function(&$blockFixedContents, $contents) {
    ...
    public function handler_format_afterFormat($sender)
    {
        ...
        $sender->content = preg_replace("/<code><\/code>/i", '<code>' . array_shift($this->inlineFixedContents) . '</code>', $sender->content);
       
        // Retrieve the contents of the block <pre> tags from the array in which they are stored.
        if (!$sender->inline) {
            $sender->content = preg_replace("/<pre><\/pre>/i", '<pre>' . array_pop($this->blockFixedContents) . '</pre>', $sender->content);
        }
    }
}
